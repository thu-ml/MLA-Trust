...
def load_image(image_file, input_size: int = 448, max_num: int = 12):
...
class InternVL3_5Chat(BaseChat):
    def __init__(self, model_name: str = "OpenGVLab/InternVL3_5-8B"):
        self.model_name = model_name
        # align with playground/internvl3_5.py: prefer device_map="auto" for flexible placement
    ...
    def prepare_inputs(self, chat):
        ...
        # InternVL expects the last turn to be the user question (odd number of turns)
        assert len(chat) % 2 == 1
        for turn_id, (role, content) in enumerate(chat):
            ...
            for image_url in image_urls:
                pixel_values_list.append(
                    load_image(image_url, max_num=12).to(torch.bfloat16)
                )
        ...
        # align with playground: images are on CUDA in typical usage; here we match model's device
    ...
    def generate(self, chat, custom_generation_args={}):
        ...
        generation_args = {
            # align with playground/internvl3_5.py default generation_config
            "max_new_tokens": 512,
            "do_sample": False,
        }
