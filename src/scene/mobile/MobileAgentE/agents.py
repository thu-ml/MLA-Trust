...
class ActionReflector(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        prompt += "---\n"
        prompt += "The attached two images are two phone screenshots before and after your last action. "
...
class ExperienceRetrieverShortCut(BaseAgent):
    def init_chat(self, sysetm_prompt: str = None) -> list:
        operation_history = []
        if sysetm_prompt is None:
...
class Perceptor:
    def __init__(self, adb_path, perception_args=None, SCREENSHOT_DIR="./screenshot"):
        (
            self.ocr_detection,
            ...
            self.vlm_model,
            self.vlm_tokenizer,
        ) = self.load_perception_models(**perception_args)
        self.adb_path = adb_path
        self.perception_args = perception_args
        self.SCREENSHOT_DIR = SCREENSHOT_DIR

    def get_perception_infos(self, temp_file=None, device_serial=None):
        screenshot_file = get_screenshot(
            self.adb_path, device_serial, SCREENSHOT_DIR=self.SCREENSHOT_DIR
        )
        ...
        text, coordinates = self.merge_text_blocks(text, coordinates)

        center_list = [
            [(coordinate[0] + coordinate[2]) / 2, (coordinate[1] + coordinate[3]) / 2]
            for coordinate in coordinates
        ]
        self.draw_coordinates_on_image(
            screenshot_file, center_list, SCREENSHOT_DIR=self.SCREENSHOT_DIR
        )
        ...
        for i in range(len(image_box)):
            self.crop(screenshot_file, image_box[i], image_id[i], temp_file=temp_file)

        images = self.get_all_files_in_folder(temp_file)
        if len(images) > 0:
            ...
            if self.perception_args["caption_call_method"] == "local":
                ...
                icon_map = self.generate_api(
                    images, prompt, caption_model=self.perception_args["caption_model"]
                )
        ...
        return perception_infos, width, height, screenshot_file
    ...
    def draw_coordinates_on_image(
        self, image_path, coordinates, SCREENSHOT_DIR="./screenshot"
    ):
        ...
        output_image_path = os.path.join(SCREENSHOT_DIR, "output_image.png")
        image.save(output_image_path)
        return output_image_path

    def crop(self, image, box, i, temp_file=None):
        image = Image.open(image)
        x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
        if x1 >= x2 - 10 or y1 >= y2 - 10:
            return
        cropped_image = image.crop((x1, y1, x2, y2))
        save_path = os.path.join(temp_file, f"{i}.png")
        cropped_image.save(save_path)

    def generate_local(self, tokenizer, model, image_file, query):
    ...
    def process_image(self, image, query, caption_model=None):
        client = OpenAI(
            base_url=self.perception_args["caption_api_url"],
            api_key=self.perception_args["caption_api_key"],
        )
        base64_image = encode_image(image)
        try:
    ...
    def generate_api(self, images, query, caption_model=None):
        icon_map = {}

        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(
                    self.process_image, image, query, caption_model=caption_model
                ): i
    ...
    def merge_text_blocks(
        ...
        for i in range(num_blocks):
            ...
            for j in range(i + 1, num_blocks):
                ...
                y_diff = (
                    sorted_coordinates_list[j][1] - sorted_coordinates_list[anchor][3]
                )
                height_anchor = (
    ...
    def load_perception_models(
        ...
        **kwargs,
        ...
        if caption_call_method == "local":
            ...
            print(
                'You must choose the caption model call function from "local" and "api"'
            )
            exit(0)
