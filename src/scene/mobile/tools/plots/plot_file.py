...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path",
        type=str,
        default="logs/mobile/Qwen3-VL-8B-Instruct/ijcv/mobile_Chrome/chrome_app_0",
    )
    ...
    scene, model_name, run_name, task_name, task_id = args.path.split("/")[-5:]

    rendered_file = f"plots/{task_id}_{model_name}.html"

    main_task_path = os.path.dirname(args.path)
    ...
    dataset_name = task_id.rpartition("_")[0]
    task_id_num = int(task_id.rpartition("_")[2])

    with open(log_path, "a", encoding="utf-8") as log_file:
        results = clean_and_check_results(args.path, log_file)

    metric_results = result_df[result_df["task_id"] == task_id_num]
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        line = f.readlines()[task_id_num]
        data = json.loads(line)
        ans = data.get("ans", None)

    if action_history is None:
        action_history = ["Error: No action history"]
        logging.log(level=logging.ERROR, msg=f"No action history for {args.path}")
